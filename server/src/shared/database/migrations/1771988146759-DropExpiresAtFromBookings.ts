...
export class DropExpiresAtFromBookings1771988146759 implements MigrationInterface {
  name = 'DropExpiresAtFromBookings1771988146759';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "temporary_bookings" ("id" varchar PRIMARY KEY NOT NULL, "created_at" datetime NOT NULL DEFAULT (datetime('now')), "updated_at" datetime NOT NULL DEFAULT (datetime('now')), "deleted_at" datetime, "user_id" varchar NOT NULL, "appointment_datetime" datetime NOT NULL, "appointment_date" date NOT NULL, "total_duration_minutes" integer NOT NULL, "status" varchar(20) NOT NULL DEFAULT ('PENDING_PAYMENT'), "payment_method" varchar(20) NOT NULL, "total_price" decimal(10,2) NOT NULL, "currency" varchar(3) NOT NULL DEFAULT ('USD'), "stripe_payment_intent_id" varchar(255), "idempotency_key" varchar(255) NOT NULL, "notes" text, "checked_in_at" datetime, "completed_at" datetime, "cancelled_at" datetime, CONSTRAINT "UQ_c827474891843af75341a82ceda" UNIQUE ("idempotency_key"), CONSTRAINT "FK_64cd97487c5c42806458ab5520c" FOREIGN KEY ("user_id") REFERENCES "users" ("id") ON DELETE NO ACTION ON UPDATE NO ACTION)`
    );
    await queryRunner.query(
      `INSERT INTO "temporary_bookings"("id", "created_at", "updated_at", "deleted_at", "user_id", "appointment_datetime", "appointment_date", "total_duration_minutes", "status", "payment_method", "total_price", "currency", "stripe_payment_intent_id", "idempotency_key", "notes", "checked_in_at", "completed_at", "cancelled_at") SELECT "id", "created_at", "updated_at", "deleted_at", "user_id", "appointment_datetime", "appointment_date", "total_duration_minutes", "status", "payment_method", "total_price", "currency", "stripe_payment_intent_id", "idempotency_key", "notes", "checked_in_at", "completed_at", "cancelled_at" FROM "bookings"`
    );
